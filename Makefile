CXXFLAGS=$(MYCXXFLAGS) -std=c++11 -I $(HOME)/local/include `pkg-config --cflags python-2.7 eigen3` -I /usr/lib/python2.7/site-packages/numpy/core/include
LDFLAGS=$(MYLDFLAGS) -L $(HOME)/local/lib -lboost_system -lboost_python `pkg-config --libs python-2.7 eigen3` -lalps -lm -lstdc++ -llapack
