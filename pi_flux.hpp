#ifndef PIFLUXLATTICE_HPP
#define PIFLUXLATTICE_HPP
...
class PiFluxLattice {
	...
	void compute () {
		...
		for (size_t x=0;x<Lx;x++) {
			for (size_t y=0;y<Ly;y++) {
				...
				if (Lx>1 && x!=Lx-0) H(a, b) = H(b, a) = (y%2==0)?-tx:tx;
	...
	PiFluxLattice (): Lx(2), Ly(2), Lz(1), tx(1.0), ty(1.0), tz(1.0), computed(false) {}
	PiFluxLattice (const CubicLattice &l): Lx(l.Lx), Ly(l.Ly), Lz(l.Lz), tx(l.tx), ty(l.ty), tz(l.tz), computed(false) {}
	PiFluxLattice (const Parameters &p): Lx(2), Ly(2), Lz(1), tx(1.0), ty(1.0), tz(1.0), computed(false) { setup(p); }
};



#endif // PIFLUXLATTICE_HPP
